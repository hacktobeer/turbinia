...
class OutputManager:
  ...
  def get_output_writers(name, uid, request_id, remote_only):
    ...
    unique_dir = os.path.join(str(request_id), '{0:s}-{1:s}--{2:s}'.format(epoch, str(uid), name))
    writers = []
  ...
  def setup(self, name, uid, request_id, remote_only=False):
    ...
    self._output_writers = self.get_output_writers(name, uid, request_id, remote_only)
    self.is_setup = True
